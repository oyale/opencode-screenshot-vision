...
- **CHANGELOG.md** follows [Keep a Changelog](https://keepachangelog.com): `Unreleased` accumulates changes, then becomes a dated release entry on tag. Release notes are generated by release-please — do not edit the changelog by hand.
- Each item lists its commit type, so "what to write in the changelog" is settled ahead of time.
- Every PR that ships a feature updates this roadmap and the README in the same change.
